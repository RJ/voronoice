...
impl ConvexBoundary for BoundingBox {
    ...
    fn intersect_line(&self, a: &Point, b: &Point) -> (Option<Point>, Option<Point>) {
        ...
        if c_x.abs() > 4. * std::f64::EPSILON {
            ...
            if right_y >= self.top() && right_y <= self.bottom() {
                f = Some(Point {
                    x: self.right(),
                    y: right_y,
                });
            ...
            if left_y >= self.top() && left_y <= self.bottom() {
                g = Some(Point {
                    x: self.left(),
                    y: left_y,
                })
        ...
        if c_y.abs() > 4. * std::f64::EPSILON {
            if c_x.abs() < 4. * std::f64::EPSILON {
                ...
                if a.x <= self.right() && a.x >= self.left() {
                    // and crosses box
                    return (
                        Some(Point {
                            x: a.x,
            ...
            if top_x <= self.right() && top_x >= self.left() {
                h = Some(Point {
                    x: top_x,
                    y: self.top(),
                })
            ...
            if bottom_x <= self.right() && bottom_x >= self.left() {
                i = Some(Point {
                    x: bottom_x,
                    y: self.bottom(),
                })
...
mod tests {
    ...
    fn intersect_line_tests() {
        ...
        assert_eq!(
            a, None,
            "No intersection expected for a parallel line to X outside of the box"
        );
        assert_eq!(
            b, None,
            "No intersection expected for a parallel line to X outside of the box"
        ...
        assert_eq!(
            a, None,
            "No intersection expected for a parallel line to Y outside of the box"
        );
        assert_eq!(
            b, None,
            "No intersection expected for a parallel line to Y outside of the box"
        ...
        assert_eq!(
            ...
            a,
            "Expected intersection with top edge"
        );
        assert_eq!(
            Some(Point {
                x: 0.0,
            ...
            b,
            "Expected intersection with bottom edge"
        );
        ...
        assert_eq!(
            Some(Point { x: -1.0, y: 0.0 }),
            b,
            "Expected intersection with left edge"
        );

        // line congruent to top edge
        let (a, b) = bbox.intersect_line(
            &Point {
                x: 0.0,
                y: bbox.top(),
            },
        ...
        assert_eq!(
            ...
            a,
            "Expected intersection with top right corner"
        );
        assert_eq!(
            Some(Point {
                x: bbox.left(),
            ...
            b,
            "Expected intersection with top left corner"
        );
        ...
        assert_eq!(
            ...
            a,
            "Expected intersection with bottom right corner"
        );
        assert_eq!(
            Some(Point {
                x: bbox.left(),
            ...
            b,
            "Expected intersection with bottom left corner"
        );
        ...
        assert_eq!(
            ...
            b,
            "Expected intersection with left bottom corner"
        );
        ...
        assert_eq!(
            Some(Point { x: 1.0, y: 0.0 }),
            a,
            "Expected intersection with middle of the right edge"
        );
        assert_eq!(
            Some(Point { x: 0.0, y: 1.0 }),
            b,
            "Expected intersection with middle of the top edge"
        );
        ...
        assert_eq!(
            Some(Point { x: -1.0, y: 0.0 }),
            a,
            "Expected intersection with middle of the left edge"
        );
        assert_eq!(
            Some(Point { x: 0.0, y: 1.0 }),
            b,
        ...
        assert_eq!(
            Some(Point { x: 0.0, y: -1.0 }),
            b,
            "Expected intersection with middle of the bottom edge"
        );
    ...
    fn project_ray_tests_centered_square() {
        ...
        assert_eq!(
            Some(Point { x: -1.0, y: 0.0 }),
            a,
            "Expected to hit left side first"
        );
        ...
        assert_eq!(
            Some(Point { x: 0.0, y: 1.0 }),
            a,
            "Expected to hit top side first"
        );
        assert_eq!(
            Some(Point { x: 0.0, y: -1.0 }),
            b,
            "And then hit the bottom side"
        );
        ...
        assert_eq!(
            Some(Point { x: 0.0, y: -1.0 }),
            a,
            "Expected to hit bottom side first"
        );
        assert_eq!(None, b, "and only that");
        ...
        let c = -0.8;
        let d = 1.0;
        let (a, b) = bbox.project_ray(
            &line(2.0, c, d),
            &direction(&line(-20.0, c, d), &line(2.0, c, d)),
        );
        assert!(
            a.is_some() && b.is_some(),
            "Expected two intersections, a: {:?}, b: {:?}",
            a,
            b
        );
        assert!(
        ...
        for &ratio in width_height_ratio.iter() {
            ...
            for i in 1..=2 {
                for j in 1..=2 {
                    ...
                    let expected_intersection_a = Point {
                        x: right,
                        y: origin.y,
                    };
                    assert!(
                        abs_diff_eq(expected_intersection_a.x, a.clone().unwrap().x, EQ_EPSILON)
                            && abs_diff_eq(
                                expected_intersection_a.y,
                                a.clone().unwrap().y,
                                EQ_EPSILON
                            ),
                        "Expected to hit right side first. found: {:?}, expected: {:?}",
                        a.unwrap(),
                        expected_intersection_a
                    );
                    let expected_intersection_b = Point {
                        x: left,
                        y: origin.y,
                    };
                    assert!(
                        abs_diff_eq(expected_intersection_b.x, b.clone().unwrap().x, EQ_EPSILON)
                            && abs_diff_eq(
                                expected_intersection_b.y,
                                b.clone().unwrap().y,
                                EQ_EPSILON
                            ),
                        "And then hit left side. found: {:?}, expected: {:?}",
                        b.unwrap(),
                        expected_intersection_b
                    );
                    ...
                    assert!(
                        ...
                        "Expected to hit left side first. found: {:?}, expected: {:?}",
                        a.unwrap(),
                        expected_intersection_a
                    );
                    let expected_intersection_b = Point {
                    ...
                    assert!(
                        ...
                        "And then hit right side. found: {:?}, expected: {:?}",
                        b.unwrap(),
                        expected_intersection_b
                    );
                    ...
                    assert!(
                        ...
                        "Expected to hit top side first. found: {:?}, expected: {:?}",
                        a.unwrap(),
                        expected_intersection_a
                    );
                    let expected_intersection_b = Point {
                    ...
                    assert!(
                        ...
                        "And then hit bottom side. found: {:?}, expected: {:?}",
                        b.unwrap(),
                        expected_intersection_b
                    );
                    ...
                    assert!(
                        ...
                        "Expected to hit bottom side first. found: {:?}, expected: {:?}",
                        a.unwrap(),
                        expected_intersection_a
                    );
                    let expected_intersection_b = Point {
                    ...
                    assert!(
                        ...
                        "And then hit top side. found: {:?}, expected: {:?}",
                        b.unwrap(),
                        expected_intersection_b
                    );
                    ...
                    let point = &Point {
                        x: origin.x + 0.3 * width,
                        y: origin.y + 0.3 * height,
                    };
                    ...
                    assert!(
                        ...
                        "And then top side. found: {:?}, expected: {:?}",
                        b.unwrap(),
                        expected_intersection_b
                    );
                    ...
                    let dir = &Point {
                        x: 0.9 * width,
                        y: ratio,
                    }; // makes sure that the incline is steep enough to pass the top-left corner on the outside
                    let (a, b) = bbox.project_ray(point, dir);
                    assert!(a.is_none() && b.is_none(), "Expected no intersection");
                }
            }
